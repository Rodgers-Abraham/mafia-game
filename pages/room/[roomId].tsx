...
interface Notification { id: string; type: string; message: string }
interface VoteTally { voteCounts: { [targetId: string]: number }; totalVoted: number; totalAlive: number }
interface MafiaTeammate { id: string; name: string; role: string; color: string }

function NotificationToast({ notifications, onDismiss }: { notifications: Notification[]; onDismiss: (id: string) => void }) {
          ...
          return (
            <div key={targetId}>
              ...
              <div className="h-1.5 bg-gray-900 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-700" style={{ width: `${(count / maxVotes) * 100}%`, background: 'linear-gradient(90deg, #8B0000, #DC143C)', boxShadow: '0 0 8px rgba(220,20,60,0.6)' }} />
              </div>
            </div>
          )
        })}
...
function ReconnectBanner({ name, secondsLeft }: { name: string; secondsLeft: number }) {
  return (
    <div className="fixed top-0 left-0 right-0 z-50 text-center py-2 spooky-title tracking-widest text-sm animate-pulse" style={{ backgroundColor: 'rgba(255,215,0,0.12)', borderBottom: '1px solid #FFD70044', color: '#FFD700' }}>
      ⚠️ {name} disconnected — {secondsLeft}s to reconnect before being removed
    </div>
  )
}
...
export default function RoomPage() {
  ...
  const [mafiaTeammates, setMafiaTeammates] = useState<MafiaTeammate[]>([])
  const [disconnectedPlayers, setDisconnectedPlayers] = useState<{ name: string; secondsLeft: number }[]>([])
  ...
  useEffect(() => {
    ...
    if (!storedData) { setError('No room data found. Please create or join a room.'); setLoading(false); return }
    const localData = JSON.parse(storedData)
    const socket = io(getSocketUrl())
    socketRef.current = socket

    socket.on('connect', () => {
      socket.emit('rejoin-room', { roomId: roomId as string, playerId: localData.playerId }, (response: any) => {
        if (response.success) {
    ...
    socket.on('game-started', (data: { room: Room }) => setRoom(data.room))

    socket.on('role-briefing', (data: { role: string }) => {
      setCurrentPlayer((prev: Player | null) => prev ? { ...prev, role: data.role as Player['role'] } : null)
    })

    socket.on('mafia-team', (data: { teammates: MafiaTeammate[] }) => setMafiaTeammates(data.teammates))

    socket.on('investigation-result', (data: { message: string }) => addNotification('investigation', data.message))
    socket.on('night-result', (data: { type: string; message: string }) => addNotification(data.type, data.message))
    socket.on('vote-updated', (data: VoteTally) => setVoteTally(data))

    socket.on('vote-result', (data: { eliminated: any; tie: boolean; jesterWin?: boolean }) => {
      if (data.tie) addNotification('default', '⚖️ The vote ended in a tie — no one was eliminated!')
      else if (data.jesterWin) addNotification('default', `🤡 ${data.eliminated?.name} was the Jester — JESTER WINS!`)
      else if (data.eliminated) addNotification('default', `💀 ${data.eliminated.name} was eliminated. They were a ${data.eliminated.role}.`)
    })

    socket.on('player-disconnected', (data: { playerName: string; gracePeriod: number }) => {
      addNotification('reconnect', `⚠️ ${data.playerName} disconnected — ${data.gracePeriod}s to reconnect`)
      setDisconnectedPlayers(prev => [...prev, { name: data.playerName, secondsLeft: data.gracePeriod }])
      let secs = data.gracePeriod
      const interval = setInterval(() => {
        secs -= 1
        setDisconnectedPlayers(prev => prev.map(p => p.name === data.playerName ? { ...p, secondsLeft: secs } : p))
        if (secs <= 0) { clearInterval(interval); setDisconnectedPlayers(prev => prev.filter(p => p.name !== data.playerName)) }
      }, 1000)
    })

    socket.on('player-reconnected', (data: { playerName: string }) => {
      addNotification('reconnect', `✅ ${data.playerName} reconnected!`)
      setDisconnectedPlayers(prev => prev.filter(p => p.name !== data.playerName))
    })

    socket.on('player-kicked', (data: { playerName: string }) => {
      addNotification('default', `🚫 ${data.playerName} was removed after failing to reconnect.`)
      setDisconnectedPlayers(prev => prev.filter(p => p.name !== data.playerName))
    })
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="text-center">
        ...
        <a href="/" className="px-6 py-3 rounded-xl border border-red-900 text-gray-400 hover:text-white transition spooky-title tracking-widest text-sm">BACK TO HOME</a>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen text-white">
      ...
      {isSpectating ? (
        ...
        <>
          {room.phase === 'lobby' && <Lobby room={room} socket={socketRef.current} currentPlayerId={currentPlayer?.id || ''} />}
          {room.phase === 'briefing' && <RoleBriefing player={currentPlayer} />}
          {room.phase === 'night' && <NightPhase room={room} player={currentPlayer} socket={socketRef.current} mafiaTeammates={mafiaTeammates} />}
